...
#define WALL_GAP_DISTANCE      360 //mm  단위
...
{
  if ((left_sonar >= 0) && (left_sonar <=220) && (right_sonar >= 0) && (right_sonar <= 220) && (front_sonar >= 0) && (front_sonar <=150))
  { //3면이 다 막힌경우
    maze_status = 4;
    Serial.println("maze_status = 4");

  }
  else if ( (left_sonar >= 0) && (left_sonar <=220) && (right_sonar >= 0) && (right_sonar <=220))
  {
    maze_status = 1;
    Serial.println("maze_status = 1");
  }
  else if ((left_sonar >= 0) && (left_sonar <=220) && (front_sonar >= 0) && (front_sonar <=180))
  {
    maze_status = 2;
    Serial.println("maze_status = 2");

  }
  else if ((right_sonar >= 0) && (right_sonar <=350) && (front_sonar >= 0) && (front_sonar <=160))
  {
    maze_status = 3;
...
void wall_collision_avoid(int base_speed)
{
  ...
  right_pwm = (base_speed - error) * 1.02;
...
void loop() {
  ...
  if (maze_status == 4)
  {
    ...
    delay(500);
    motor_A_control(HIGH, 110);   //오른쪽 전진
  ...
  else if (maze_status == 2)
  {
    ...
    motor_B_control(HIGH,102);    //왼쪽 전진
    delay(495);
    motor_A_control(HIGH, 0);
    motor_B_control(LOW, 0);
    delay(500);
