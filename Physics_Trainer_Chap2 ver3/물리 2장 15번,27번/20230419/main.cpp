...

{
	...
	if (solution == SHOW)
	{
		...
		printf("우리가 구해야 하는건 가속도 이므로 a(가속도) = (v^2 - v0^2) / 2 * Δx의 형태로 바꾸어 줍니다.\n\n");

		printf("먼저 속도의 단위를 km/h 를 m/s 로 바꿔야 합니다.\n");
		printf("1 km = 1000 m로 *1000 을(를) 해주고 1h = 3600 s 이므로 /3600 s 을(를) 해줍니다.\n");
		velocity_m = pow(velocity * m, 2) / pow(s, 2);
		printf("그러므로 %5.2lf km/h = %5.2lf * 1000 / 3600 m/s = %5.2lf m/s 입니다.\n\n", velocity, velocity, velocity_m);

		printf(" v^2은 나중속도 이므로 (%5.2lf m/s)^2 이고,\n", velocity);   // 120은 나중에 자동을 바뀔 값이므로 변수로 바꿀것
		printf(" v0^2은 처음속도 이므로 (0)^2 입니다.\n");
		velocity_1 = velocity_m - pow(velocity0, 2);
		printf(" v^2 - v0^2의 값은 %5.2lf입니다.\n\n", velocity_1);

		printf("2 * a * x 에서 x는 거리(%5.2lf m)을 말한다.\n\n", distance);
		a_1 = num * distance;
		a = velocity_1 / a_1;
	...
	if (answer == SHOW)
	{
		...
		printf("(%5.2lf)^2 - (%5.2lf)^2 = 2 * a * %5.2lf 입니다.\n", velocity, velocity0, distance);
		printf("가속도(a)에 대한 식으로 정리하면 a = %5.2lf 입니다.\n", a);
		printf("답: a = %5.2lf m/s^2 \n\n", a);
	...
	if (solution == SHOW)
	{
		...
		printf("(v = %5.2lf) , (v0 = %5.2lf) , (a = %5.2lf) 입니다.\n", velocity, velocity0, a);
		printf(" 시간을 구해야 하므로 시간(t)에 대한 식으로 정리하면 t = (v - v0) / a 가 됩니다.\n");
	...
	if (answer == SHOW)
	{
		...
		printf("공식 v = v0 + a * t 에 대입하면\n");
		printf("(%5.2lf) = (%5.2lf) + (%5.2lf) * t 가 되고\n", velocity, velocity0, a);
		printf(" 시간(t)에 대한 식으로 정리하면 t = (v - v0) / a 가 됩니다.\n");
		printf("답: %5.2lf s.\n\n", t);
...
void Excersize_2_27(int solution, int answer)
{
	...
	if (solution == SHOW)
	{
		...
		printf("관련 공식은 y = v0 * t - 1 / 2 * g * t ^ 2 입니다.\n");
		printf("여기서 ( y(높이) = 75 m ) , ( v0(처음 속도) = 12.0 m/s ) , ( g(중력) = 9.8 ) 입니다.\n");
		printf("식에 대입하면 -75 = 12.0 t - 1/2 * 9.8 t^2 입니다.\n");
		printf("이때 y 에 (-)가 붙은 이유는 돌이 떨어지는 방향이 아래 이기 때문에 (-)를 곱해줘야 합니다.\n\n");

		printf("시간(s)를 구하려면 위의 식을 이차방정식으로 정리 해야 합니다.\n");
		printf("이차방정식 공식은 t^2 + t + num = ans 입니다.\n");

		printf("-75 = 12.0 t - 1/2 * 9.8 t^2 이 식을 t^2 + t + num = ans 형태로 정리하면 답이 나옵니다.\n\n");
	...
	if (answer == SHOW)
	{
		...
		t_1 = v0;
		num = height;
		printf("t^2 + t + num = ans 에서\n");
		printf("t^2 은: %5.2lf 이고, t 는: %5.2lf , z 는: %5.2lf 이므로\n", t_square, t_1, num);
		printf("t^2 + t + num = ans 에다가 대입을 하면\n");
		d = t_1 * t_1 - 4.0 * t_square * num;
		e = sqrt(d);
		((-t_1 + e) / (2.0 * t_square) >= (-t_1 - e) / (2.0 * t_square));
		printf("(a) 얼마 후에 돌이 절벽 아래 지면에 도달하겠는가 에서 걸린 시간은\n");
		printf("답: t = %5.2lf s\n\n", (-t_1 - e) / (2.0 * t_square));
	...
	if (solution == SHOW)
	{
		...
		printf("에서 총 거리를 구하려면 절벽높이 %5.2lf m + 최고점을 찍은 거리(m)도 필요하다.\n");
		printf("관련 공식은 v^2 - v0^2 = 2 * a * Δy 입니다.\n");
		printf("위 식을 변혀아면 Δy = v^2 / 2 * g 가 됩니다.\n");
		printf("(v 처음속도 = %5.2lf) , (a 는 g(중력가속도) 이므로 = %5.2lf) 입니다.\n\n", velocity, gravity_1);
		printf("여기에 기본 절벽 높이인 %5.2lf m를 더해줘야 합니다.\n\n", height);
	...
	if (answer == SHOW)
	{
		...
		printf("공식 Δy = v^2 / 2 * g 에 대입하면 \n");
		printf("Δy = (%5.2lf)^2 / 2 * (%5.2lf m/s) 입니다\n.", velocity, gravity_1);
		ans_c = pow(velocity, 2) / (2 * gravity_1);
		printf("최고점 까지의 거리는 %5.1lf m 입니다.\n\n", ans_c);
		printf("(최고점까지의 거리 %5.1lf m) * 2 + (절벽 높이 %5.2lf m) 하면 답이 나옵니다.\n\n", ans_c, height);
